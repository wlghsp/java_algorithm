...
우리 안은 1차원 배열로 생각하며, 총 N(1 ≤ N ≤ 100000)개의 얼음 양동이들이 
xi(0 ≤ xi ≤ 1,000,000)좌표마다 놓여 있고 각 양동이 안에는 gi(1 ≤ gi ≤ 10,000)씩의 얼음이 들어 있다. 
...
public class Boj10025_게으른백곰 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            arr[p] = w; // p라는 좌표에 얼음이 w만큼 들어 있음. 
        }

        int sum = 0;
        int max = 0;

        int window = 1 + (2* k); // 슬라이딩 윈도우의 크기, k 거리만큼 닿고 + 1을 해주면 그 크기임.
        
        boolean flag = false; // sum, max 비교연산을 막기 위한 
        // flag의 역할은 처음 부분 배열이 완성되지 않았는데 
        // sum과 max값을 비교하는 것을 방지해주는 역할

        for (int i = 0; i < 1000000; i++) {
            // 초반 k-1 범위 전까지는 sum을 max랑 비교하면 안됨. 
            if (i == window-1) flag = true; //  i가 window크기일때부터 비교하도록 true로 바꿈 
            if (i >= window) sum -= arr[i-window]; // window 크기를 벗어났으므로 앞 인덱스 부분은 삭제한다.
            sum += arr[i]; // 추가된 값을 더해준다. 
            if (sum > max && flag) max = sum;
        }
